...
以 MikroTik **CRS812-8DS-2DQ-2DDQ-RM** 交换机（RouterOS）为例。完整端口：8× **DS 口**（50G，本集群未使用）+ 2× **DQ 口**（QSFP56，200G）+ 2× **DDQ 口**（QSFP-DD，200G）。本集群只用其中 4 个 RoCE 接口：2 个 **DQ 口**（`qsfp56-1-1` / `qsfp56-2-1`）+ 2 个 **DDQ 口**（`qsfp56-dd-1-1` / `qsfp56-dd-2-1`）：
