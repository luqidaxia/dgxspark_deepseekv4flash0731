#!/bin/bash
# ============================================================
# DeepSeek V4 Flash 4 节点 TP=4 — node01 (head, rank 0)
# 用法: bash node01.sh
# 前提: 1) 镜像已就位  2) 模型权重已就位  3) config.sh 已配置
# 启动顺序: 先启动 3 个 worker (node02/03/04)，最后启动 head (node01)
# ============================================================
set -euo pipefail
...
echo "========================================"
echo " DeepSeek V4 Flash - $NODE_NAME (head, rank $NODE_RANK)"
echo "========================================"
...
# --- 5. 启动 head 容器 ---
echo "🚀 启动 $NODE_NAME (head, rank $NODE_RANK)..."
echo "   MASTER_ADDR=$MASTER_ADDR  TP=$TP_SIZE  NNODES=4"
echo "   API 端口: $PORT"
echo ""
...
echo ""
echo "✅ $NODE_NAME (head) 已启动 (容器: vllm_anemll)"
echo ""
echo "📊 查看日志: docker logs -f vllm_anemll"
echo "🔌 API 地址: http://${NODE01_IP}:${PORT}/v1"
echo ""
echo "⏳ 等待模型加载完成 (约 5 分钟) 后即服务就绪..."
