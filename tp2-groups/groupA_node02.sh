#!/bin/bash
# ============================================================
# DeepSeek V4 Flash — TP=2 分两组 — 组A: node02 (worker, rank 1)
# 组A = node01 + node02 (TP=2, nnodes=2)
# 启动顺序: worker 先启动，等待 head (node01) 建连
# ============================================================
set -euo pipefail
...
NODE_RANK=1
VLLM_HOST_IP="10.10.10.102"
MASTER_ADDR="$GROUP_A_MASTER_ADDR"
MASTER_PORT="$GROUP_A_MASTER_PORT"
API_PORT="$GROUP_A_API_PORT"
HEADLESS="--headless"

echo "========================================"
echo " DeepSeek V4 Flash - 组A node02 (worker, rank 1, TP=2)"
echo "========================================"
...
# --- 启动 worker 容器 ---
echo "🚀 启动 组A node02 (worker, rank 1, TP=2)..."
...
echo ""
echo "✅ 组A node02 (worker, rank 1, TP=2) 已启动 (容器 vllm_anemll)"
echo "📊 日志: docker logs -f vllm_anemll"
echo "⏳ 等待 head (node01) 启动后完成 NCCL 建连..."
