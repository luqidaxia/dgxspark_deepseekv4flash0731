#!/bin/bash
# ============================================================
# DeepSeek V4 Flash — TP=2 分两组 — 组B: node04 (worker, rank 1)
# 组B = node03 + node04 (TP=2, nnodes=2)
# 启动顺序: worker 先启动，等待 head (node03) 建连
# ============================================================
set -euo pipefail
...
echo "========================================"
echo " DeepSeek V4 Flash - 组B node04 (worker, rank 1, TP=2)"
echo "========================================"
...
# --- 启动 worker 容器 ---
echo "🚀 启动 组B node04 (worker, rank 1, TP=2)..."
...
echo ""
echo "✅ 组B node04 (worker, rank 1, TP=2) 已启动 (容器 vllm_anemll)"
echo "📊 日志: docker logs -f vllm_anemll"
echo "⏳ 等待 head (node03) 启动后完成 NCCL 建连..."
